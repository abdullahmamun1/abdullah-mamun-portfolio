...
export function Experience() {
  return (
    ...
    >
      <Reveal>
        <SectionHeading prefix="04" eyebrow="experience" title="Where I've worked" />
      </Reveal>
      ...
      {experience.length === 0 ? (
        <Reveal delay={0.05}>
          <motion.div
            ...
            className="card p-8 transition-colors hover:border-[var(--accent)]"
          >
            <p className="font-mono text-sm text-[var(--accent)]">no formal roles yet</p>
          ...
          {experience.map((item, i) => (
            <Reveal key={item.role + item.org} delay={i * 0.06}>
              <motion.div
                whileHover={{ y: -4 }}
                transition={{ type: "spring", stiffness: 260, damping: 22 }}
                className="card p-6 transition-colors hover:border-[var(--accent)]"
              >
